...
- increase limit
- reduce parallel fan-out (finished agents shut down automatically once they reach a final status)

### `followup_task` Does Not Start a Turn

- `followup_task` only starts a turn when the target agent is idle
- use `send_message` to queue work without starting a turn
- use `interrupt_agent` first to interrupt a running turn

### `wait_agent` Keeps Timing Out
